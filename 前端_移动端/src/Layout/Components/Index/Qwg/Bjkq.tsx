...
const Bjkq: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // 自动重新计算当前列表中的状态
        const newKuangke = studentList.filter(item => item.cate === '2').length;
        const newQingjia = studentList.filter(item => ['3', '4'].includes(item.cate)).length;
        ...
        <div className={Style.BjkqPage}>
            ...
            <div className={Style.BjkqPage_footer}>
                <div className={Style.BjkqPage_footer_item}>
                    ...
                    <div className={Style.BjkqPage_footer_item_bottom}>应到{total}人，实到{Number(total) - Number(kuangke) - Number(qingjia)}人，旷课{kuangke}人，请假{qingjia}人</div>
                </div>
