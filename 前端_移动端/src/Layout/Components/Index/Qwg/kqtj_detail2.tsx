...
const App: React.FC = () => {
  ...
  const totalStudents = classData.reduce((acc, curr) => acc + curr.total, 0);
  const totalPresent = classData.reduce((acc, curr) => acc + curr.present, 0);
  ...
  const exportToExcel = () => {
    ...
    // 准备数据（示例增加时间维度）
    const data = [
      ['统计周期', dayjs(selectDate).format('YYYY-MM-DD'), '', '导出时间', dayjs().format('YYYY-MM-DD HH:mm')],
      ['班级名称', '应到人数', '实到人数', '出勤率', '病假', '事假', '旷课'],
      ...classData.map(item => [
        item.name,
        item.total,
        item.present,
    ...
    saveAs(new Blob([wbout], { type: "application/octet-stream" }),
      `考勤统计_${dayjs().format('YYYYMMDD_HHmm')}.xlsx`)
  }
            ...
            <div className="px-4">
              {viewMode === 'list' ? (
                <div className="space-y-4">
                  {classData
                    .filter(classInfo => selectedGrade === '全部年级' || classInfo.name.startsWith(selectedGrade))
                    .map((classInfo, index) => (
